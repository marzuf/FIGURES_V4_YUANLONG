########################################################################################################################################################################################
startTime <- Sys.time()
cat(paste0("> Rscript look_TAD_expression_withRank_withMutStatus_facet.R\n"))

script_name <- "look_TAD_expression_withRank_withMutStatus_facet.R"
...
# Rscript look_TAD_expression_withRank_withMutStatus_facet.R ENCSR489OCU_NCI-H460_40kb TCGAluad_mutKRAS_mutEGFR chr17_TAD162
# Rscript look_TAD_expression_withRank_withMutStatus_facet.R ENCSR489OCU_NCI-H460_40kb TCGAluad_nonsmoker_smoker chr10_TAD16
# Rscript look_TAD_expression_withRank_withMutStatus_facet.R ENCSR489OCU_NCI-H460_40kb TCGAluad_nonsmoker_smoker chr17_TAD162
...
outFolder <- file.path("LOOK_TAD_EXPRESSION_WITHRANK_WITHMUTSTATUS_FACET")
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", tad_to_plot, "_allSamples_exprValues_boxplot_condBox_condmutJitter.", plotType))
ggsave(plot = p_var_boxplot, filename = outFile, height=myHeightGG, width = myWidthGG*1.3)
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", tad_to_plot, "_allSamples_exprValues_boxplot_condmutBox_condmutJitter.", plotType))
ggsave(plot = p_var_boxplot, filename = outFile, height=myHeightGG, width = myWidthGG*1.3)
cat(paste0("... written: ", outFile, "\n"))
...
# ggplot(withRank_toplot_dt2, aes(x=symbol_lab, y=value_log10, fill=interaction(cond,cond_sh), dodge=cond_sh)) +
  # stat_boxplot(geom ='errorbar')+
  # geom_boxplot()+
  # geom_jitter(position=position_jitterdodge(), alpha=0.7, shape=21)
  


p_var_boxplot <-  ggplot(withRank_toplot_dt2, aes(x = symbol_lab, y = value_log10, group=group)) + 
  # geom_boxplot( notch = TRUE, outlier.shape=NA)+
  # geom_jitter(aes(colour = all_cond), position=position_jitterdodge(), alpha=0.7)+
  
  
  geom_boxplot(aes(colour=cond), notch = TRUE, outlier.shape=NA)+
  geom_jitter(aes(fill=all_cond), position=position_jitterdodge(), alpha=0.7, shape=21)+
  ...
  #                    labels=setNames(c(paste0(cond1, "-wt"),paste0(cond1, "-mut"),  paste0(cond2, "-wt"), paste0(cond2, "-mut")),
  #                                    c(paste0(cond1, ".noMut"),paste0(cond1, ".withMut"),  paste0(cond2, ".noMut"), paste0(cond2, ".withMut")))) + 
  scale_color_manual(values=setNames(c(col1, col2),
                                     c(cond1,cond2)))+
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", tad_to_plot, "_allSamples_exprValues_boxplot_condBox_condmutJitter_nofill.", plotType))
ggsave(plot = p_var_boxplot, filename = outFile, height=myHeightGG, width = myWidthGG*1.3)
cat(paste0("... written: ", outFile, "\n"))
...
p_var_boxplot <-  ggplot(withRank_toplot_dt2, aes(x = symbol_lab, y = value_log10)) + 
  geom_boxplot(aes(colour=all_cond),notch = TRUE, outlier.shape=NA)+
  geom_jitter(aes(colour = all_cond, fill=all_cond), position=position_jitterdodge(), alpha=0.7)+
  # geom_jitter(aes(colour = cond, shape=cond_sh), position=position_jitterdodge())+
  ggtitle(paste0(hicds, " - ", exprds), subtitle = paste0(subTit))+
  scale_x_discrete(name=my_xlab)+
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", tad_to_plot, "_allSamples_exprValues_boxplot_condmutBox_condmutJitter_nofill.", plotType))
ggsave(plot = p_var_boxplot, filename = outFile, height=myHeightGG, width = myWidthGG*1.3)
cat(paste0("... written: ", outFile, "\n"))

# x=combn(levels(withRank_toplot_dt2$all_cond),2)
# my_comparisons <- foreach(i=1:ncol(x)) %do% x[,i]
# my_comparisons = list(c("mutKRAS.noMut", "mutEGFR.noMut"),
#                       c("mutKRAS.withMut", "mutEGFR.noMut")
#                       )
my_comparisons = list(c("mutKRAS.noMut", "mutEGFR.noMut"),
                      c("mutKRAS.withMut", "mutEGFR.noMut")
                      )
...
my_xlab <- "TAD genes (ordered by start positions) - with Wilcoxon test"


p_var_boxplot <-  ggboxplot(withRank_toplot_dt2, 
                            x = "all_cond", y = "value_log10") + 
  facet_wrap(~symbol_lab, switch='x')+
  ...
  stat_compare_means(aes(label = paste0(..method.., "\n", "p =", ..p.format..)), method="wilcox.test", comparisons = my_comparisons) +
  ...
  theme( 
    ...
    axis.text.y = element_text(color="black", hjust=1,vjust = 0.5, size=12),
    # axis.text.x =element_text(color="black", hjust=0.5,vjust = 0.5, size=12, face="bold"),
    strip.text.x = element_text(color="black", hjust=0.5,vjust = 0.5, size=12, face="bold"),
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", tad_to_plot, "_allSamples_exprValues_boxplot_condmutBox_condmutJitter_facetBoxplot_pval.", plotType))
